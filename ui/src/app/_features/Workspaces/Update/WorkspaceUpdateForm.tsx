    ...
    <div className='w-64'>
      <form onSubmit={handleSubmit(onSubmit)} className='flex w-full flex-col gap-4'>
        <Controller
          name='name'
          control={control}
          render={({ field }) => <Form.Input title='Name*' size='lg' value={field.value} onChange={field.onChange} />}
        />
        ...
        <div className='flex items-center gap-2'>
          <Button title='Save' primary type='submit' loading={isPending} disabled={!canUpdateForm || isPending} />
          <Button title='Cancel' onClick={handleHideForm} className='bg-gray-200 hover:bg-gray-300' />
        </div>
      </form>
    </div>
  )
