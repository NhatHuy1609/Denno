...
export function evaluateDynamicDropdownItems<TBase, TDynamic = {}>(
  ...
  return items.map((item) => {
    ...
    propertyEvaluators.forEach((evaluator) => {
      ...
      const result = policyEngine.canWithReason(evaluator.action, evaluator.resource, itemContextData, resourceData)

      // Map policy result to property value
      const propertyValue = evaluator.mapPolicyResult ? evaluator.mapPolicyResult(result) : result.allowed
